...
function ApplyVertexNoise(noise, particle){
	...
	/*var dx = particle.x-pos[0];
	var dy = particle.y-pos[1];

	var vx =- dy*noise.speed[0];
	var vy = dx*noise.speed[1];

    var factor = 1/(1+(dx*dx+dy*dy)/noise.scale)

	particle.vx += (vx-particle.vx)*factor
	particle.vy += (vy-particle.vy)*factor*/
